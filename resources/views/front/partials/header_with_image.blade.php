<div class="section-main">
    <a href="/">
        <img
            style="height: 150vh;"
            class="absolute main-image bg-auto bg-cover bg-contain"
            src="https://auto-help-spb.ru/images/top/index.jpg"
            alt=""
        >
    </a>
    <div class="all-content px-12">
        <header class="pt-2">
            <div id="callContainer" class="hidden ">
                <div class="formBtn-container border-4 border-black  bg-white text-center  py-8 flex-col items-center justify-center absolute">
                    <i id="closeBtn" style="font-size: 24px; right: 20px;" class="fas fa-times fixed top-1 cursor-pointer "></i>
                    <h1 class="mt-6 mb-4 font-extrabold text-xl">Заказать звонок</h1>

                    <form action="" class="flex-col flex-wrap justify-center items-center">
                        <input class="formBtn-input bg-gray-300 p-2 pr-16 outline-none m-4 " type="text" placeholder="Ваше имя">
                        <input class="formBtn-input bg-gray-300 p-2 pr-16 outline-none m-4" type="tel" name="" id="" placeholder="Ваш телефон*">
                        <input class="formBtn-input bg-blue-700 text-white hover:bg-blue-900 m-4  p-2 px-6 outline-none" type="submit" value="Отправить">
                    </form>

                </div>
            </div>
            <div class="header-container">
                <div class="header-main flex text-white items-center justify-between ">

                    <div class="logo cursor-pointer">
                        <a href="/pages/index.html">
                            <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAASwAAABwCAYAAACkRk1NAAAACXBIWXMAA
                    " alt="">
                        </a>
                    </div>
                    <div class='hidden lg:flex'>
                        <i class="fas fa-map-marker-alt pr-2"></i>
                        <p>{{ setting('site.address') }}</p>
                    </div>
                    <div class='flex hidden lg:flex'>
                        <i class="fas fa-clock pr-2"></i>
                        <p>{{ setting('site.work_time') }}</p>
                    </div>
                    <div class='flex'>
                        <i class="fas fa-phone-alt pr-2"></i>
                        <div class="flex-col">
                            <p>{{ setting('site.phone_number_one') }}</p>
                            <p>{{ setting('site.phone_number_two') }}</p>
                        </div>
                    </div>
                </div>
                <div class="header-menu container text-white cursor-pointer pr-48">
                    <ul class="flex justify-between p-4 items-center">
                        <li id="burger" class="burger"><i class="fas fa-bars text-orange-600"></i></li>
                        <li class="header-menu__item header-menu__parentitem">
                        <li  class="header-menu__link header-menu__parentlink cursor-default hide-nav">
                            Услуги
                        </li>
                        <div class="header-submenu z-10 -mt-8">
                            <ul class="header-submenu__in">
                                @forelse($categories as $category)
                                    <li class="header-submenu__item">
                                        <a href="{{ route('website.category', $category->id) }}" class="header-submenu__link">
                                            <i>
                                                <img src="{{ $category->getIconUrl()  }}" alt="" class="lazy">
                                            </i>
                                            {{ $category->name }}
                                        </a>
                                    </li>
                                @empty
                                    <li class="header-submenu__item">
                                        <a href="#" class="header-submenu__link">
                                            Пока пусто
                                        </a>
                                    </li>
                                @endforelse
                            </ul>
                        </div>


                        <li class="hide-nav hover:text-orange-600">
                            <a href="{{ route('website.prices') }}"> ЦЕНЫ </a>
                        </li>
                        <li class="hide-nav hover:text-orange-600">
                            <a href="{{ route('website.reviews') }}"> ОТЗЫВЫ </a>
                        </li>
                        <li class="hide-nav hover:text-orange-600">
                            <a href="{{ route('website.contacts') }}"> КОНТАКТЫ </a>
                        </li>
                        <li
                            id="callBtn"
                            class="border-2 border-white p-3 hover:bg-blue-700 hover:border-blue-700 transition duration-500"
                        >
                            Заказать звонок
                        </li>
                    </ul>
                </div>
            </div>

        </header>







        <div class="header font-bold lg:text-5xl md:text-3xl sm:text-xl mt-16">
            <h1 class="text-white">{{ $category->inner_title }}</h1>
            <h1 class="text-white"> ОТ <span class="text-orange-600"> {{ $category->price_from }} РУБЛЕЙ </span></h1>
        </div>
        <div id="callMaster" class="hidden">
            <div class="callContainer bg-white text-center border-4 border-black  py-8 flex-col items-center justify-center fixed">
                <i id="closeMasterBtn" style=" font-size: 24px; right: 20px;" class="fas fa-times absolute top-1 cursor-pointer "></i>
                <h1 class="mt-6 mb-4 font-extrabold text-2xl">Вызвать мастера</h1>

                <form action="" class="flex-col flex-wrap justify-center items-center">
                    <input class="formBtn-input bg-gray-300 p-2 pr-16 outline-none m-4 " type="text" placeholder="Ваше имя">
                    <input class="formBtn-input bg-gray-300 p-2 pr-16 outline-none m-4" type="tel" name="" id="" placeholder="Ваш телефон*">
                    <textarea style="height: 100px;" class="bg-gray-300 p-2 pr-16 ml-4 outline-none m-4" name="" id="" cols="30" rows="10" placeholder="Ваше сообщение"></textarea>
                    <input class="formBtn-input bg-blue-700 text-white hover:bg-blue-900 m-4  p-2 px-6 outline-none" type="submit" value="Отправить">
                </form>

            </div>
        </div>
        <ul class="content flex-col items-center justify-evenly text-white mt-10">

            @foreach($category->lists as $list)
                <li>
                    <i class="fas fa-angle-double-right pr-4 text-orange-600 mt-4"></i>
                    {{ $list->text }}
                </li>
            @endforeach

            <button id="callMasterBtn" style="font-size: 16px;" class="mt-16 mb-8 py-4 px-8 bg-blue-600 hover:bg-blue-700  transition duration-200">Вызват мастера</button>
        </ul>

    </div>
</div>

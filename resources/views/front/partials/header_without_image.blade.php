<div class="section-main bg-black">
    <div class="all-content px-12">
        <header class="pt-2">
            <div class="header-container">
                <div class="header-main flex text-white items-center justify-between ">

                    <div class="logo cursor-pointer">
                        <a href="/pages/index.html"><img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAASwAAABwCAYAAACkRk1NAAAACXBIWXMAA
                     " alt=""></a>
                    </div>
                    <div class='hidden lg:flex'>
                        <i class="fas fa-map-marker-alt pr-2"></i>
                        <p>Работаем по Санкт-Петербургу и Ленинградской области</p>
                    </div>
                    <div class='flex hidden lg:flex'>
                        <i class="fas fa-clock pr-2"></i>
                        <p>Работаем 24 часа</p>
                    </div>
                    <div class='flex'>
                        <i class="fas fa-phone-alt pr-2"></i>
                        <div class="flex-col">
                            <p>+7(812)629-03-78</p>
                            <p>+7(911)922-03-78</p>
                        </div>
                    </div>
                </div>
                <div class="header-menu container text-white cursor-pointer px-8">
                    <ul class="flex justify-between p-4 items-center">
                        <li class="burger"><i class="fas fa-bars text-orange-600"></i></li>
                        <li class="header-menu__item header-menu__parentitem">
                        <li  class="header-menu__link header-menu__parentlink cursor-default hide-nav">
                            Услуги
                        </li>
                        <div class="header-submenu z-10" style="opacity: 2;">
                            <ul class="header-submenu__in">
                                @foreach($categories as $category)
                                <li class="header-submenu__item">
                                    <a href="#" class="header-submenu__link">
                                        <i>
                                            <img src="{{ $category->getIconUrl() }}" alt="" class="lazy">
                                        </i>
                                        {{ $category->name }}
                                    </a>
                                </li>
                                @endforeach
                            </ul>
                        </div>
                        </li>
                        <li class="hide-nav hover:text-orange-600"><a href="{{ route('website.prices') }}"> ЦЕНЫ </a></li>
                        <li class="hide-nav hover:text-orange-600"><a href="{{ route('website.reviews') }}"> ОТЗЫВЫ </a></li>
                        <li class="hide-nav hover:text-orange-600"><a href="{{ route('website.contacts') }}"> КОНТАКТЫ </a></li>
                        <li class="border-2 border-white p-3 hover:bg-blue-700 hover:border-blue-700 transition duration-500">Заказать звонок</li>
                    </ul>
                </div>
            </div>
        </header>
    </div>
</div>
